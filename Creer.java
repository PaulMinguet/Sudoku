...
public class Creer extends JComponent implements MouseListener{
	...
	public void mouseClicked(MouseEvent e){
		...
		this.clicX < ((this.getWidth()-this.tailleCase*9)/2-this.tailleCase*3)/2+3*this.tailleCase && this.clicY > (this.getHeight()-this.tailleCase*3)/2 && this.clicY < (this.getHeight()-this.tailleCase*3)/2+4*this.tailleCase){
			...
			/*---------------------------------------------------------------------------------------------*/
				/*for(i = 0; i < 9; i++){
					if(this.tableau.getValeur(0,i,posX) == valAdd){
						this.erreur = 1;
						System.out.println(this.tableau.getValeur(0,i,posX));
					}else{
						this.erreur = 0;
					}
				}
				...
				for(j = 0; j < 9; j++){
					if(this.tableau.getValeur(0,posY,j) == valAdd){
						this.erreur = 1;
						System.out.println(this.tableau.getValeur(0,posY,j));
					}else{
						this.erreur = 0;
					}
				}
